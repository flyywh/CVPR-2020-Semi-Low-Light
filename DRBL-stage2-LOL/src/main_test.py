...
if checkpoint.ok:
    ...
    my_model.model.load_state_dict(torch.load('./pretrain/model_20.pt'))

    args.model = 'RECOMPOSE'
    my_recomp = model.Model(args, checkpoint)
    my_recomp.model.load_state_dict(torch.load('./pretrain/model_R_20.pt'))

    args.model = 'DISCRIMINATOR'
    my_dis = model.Model(args, checkpoint)
    my_dis.model.load_state_dict(torch.load('./pretrain/model_D_20.pt'))
